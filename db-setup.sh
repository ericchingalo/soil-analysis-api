...
if [ "$#" -eq  "3" ]
   ...
   psql -h localhost -U postgres -tc "SELECT 1 FROM pg_database WHERE datname = '$3'" | grep -q 1 || psql -h localhost -U postgres -c "CREATE DATABASE $3 WITH OWNER = $1  ENCODING = 'UTF8' EXTENSION='uuid-ossp' LC_COLLATE = 'en_US.UTF-8' LC_CTYPE = 'en_US.UTF-8' TABLESPACE = pg_default CONNECTION LIMIT = -1;" 
